"""
Multi-product passive market-maker (round 5) — v3.

3-day Python BT:  v2 99,025 → v3 147,189  (+48,164, +48.6%).
Rust BT 147,188 → engines agree within 0.01% (sanity check passed).
Per-day all positive: D2 40,799 / D3 44,376 / D4 62,014.

Two changes that worked:

  A. Universe expansion 14 → 20 products. Surveyed all 50 products by
     spread/vol ratio + FDR-IC OBI signal; added 6 winners:
         PEBBLES_S
     ...
     Excluded 3 candidates that bled in 3-day backtest despite passing the
     structural gates (PEBBLES_M −9,674, OXYGEN_SHAKE_MINT −3,911,
     PANEL_4X4 −5,936). Their spread/vol ratio is borderline (~0.88-0.90)
     and single-day blowups (e.g. PANEL_4X4 D4 −16,585) dominate the
     small 3-day sample. Worth re-evaluating with more data.

  B. Vol-aware dynamic half-spread (Avellaneda-Stoikov): when realised σ
     exceeds vol_cap, half_spread widens to K_VOL · σ. Floors at the
     per-product fixed minimum during calm regimes. K_VOL = 0.5; tighter
     than the strat_mm_snackpack template (K_VOL = 1.65) because most
     v3 products quote at min_edge = 3-7 already.

Two strategies tested and rejected:

  C. Take-and-make. Both reference points (OBI-shifted fair, EWMA mu)
     bled −600k+ on 3-day BT. Mechanism: mu lags trend, so condition
     `ask < mu − threshold` fires during downtrends and we BUY the
     falling market. Take-and-make on a single product needs a
     cross-product reference (lead-lag pair, basket fair) — disabled.

  D. Microprice as fair anchor for products without OBI signal. Microprice
     re-evaluates every tick on book sizes; replacing the slow EWMA mu
     with it caused fair-chase adverse fills (PEBBLES_L: +5,301 → −2,261).
     Microprice is a current-tick reference, not a fair-value estimator;
     it works as an OBI-overlay shift, not as the level anchor. Disabled.

Risk caps unchanged (POSITION_LIMIT = 10, QUOTE_SIZE = 10).
"""
...
# obi_ic = signed IC (sign = direction), obi_sig = "obi_l1"|"obi_l3"|None.
PRODUCTS: dict[str, dict[str, Any]] = {
    ...
    # --- v3 additions: spread/vol ≥ ~0.9 + FDR-sig OBI, 3-day net positive ---
    "GALAXY_SOUNDS_PLANETARY_RINGS":  {"half_spread": 6, "obi_sig": "obi_l1","obi_ic": +0.059,  "vol_cap": 10.70},
    "OXYGEN_SHAKE_GARLIC":            {"half_spread": 6, "obi_sig": "obi_l1","obi_ic": +0.066,  "vol_cap": 11.83},
    "PANEL_1X2":                      {"half_spread": 5, "obi_sig": "obi_l1","obi_ic": +0.057,  "vol_cap":  8.91},
    "TRANSLATOR_ASTRO_BLACK":         {"half_spread": 3, "obi_sig": "obi_l3","obi_ic": -0.042,  "vol_cap":  9.30},
    "TRANSLATOR_ECLIPSE_CHARCOAL":    {"half_spread": 3, "obi_sig": "obi_l3","obi_ic": -0.036,  "vol_cap":  9.73},
    "PEBBLES_S":                      {"half_spread": 4, "obi_sig": "obi_l1","obi_ic": +0.036,  "vol_cap": 14.83},
    # Dropped — 3-day net negative (PEBBLES_M -9,674, OXYGEN_SHAKE_MINT -3,911,
    # PANEL_4X4 -5,936). Includable if structural conviction grows; FDR-IC is
    # there, but spread/vol ratio is borderline (~0.88-0.90) and single-day
    # blowups (e.g. PANEL_4X4 D4 -16,585) dominate the 3-day sample.
}
...
WARMUP: int = 30
INV_SKEW: float = 0.5
OBI_GAIN_PER_IC: float = 80.0
OBI_GAIN_CAP: float = 12.0
OBI_VOL_BOOST: float = 0.5
OBI_SIZE_THRESHOLD: float = 0.30
OBI_SIZE_TILT: float = 0.4
INV_TAPER_START: float = 0.5
STOP_DEV: float = 50.0
VOL_WINDOW: int = 50

# Take-and-make layer.
K_VOL: float = 0.5          # dynamic half-spread coefficient (Avellaneda-style)
TAKE_EDGE_TICKS: int = 2    # take-and-make threshold (currently disabled)
TAKE_SIZE: int = 3
...
def microprice_of(depth: OrderDepth, bid: int, ask: int) -> float:
    bsz = depth.buy_orders.get(bid, 0)
    asz = -depth.sell_orders.get(ask, 0)
    tot = bsz + asz
    if tot <= 0:
        return (bid + ask) / 2.0
    return (bsz * ask + asz * bid) / tot
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product, cfg in PRODUCTS.items():
            ...
            anchor = mu  # microprice fallback disabled — drove anchor-chase bleeds in early v3.

            inv_shift = INV_SKEW * pos
            fair = anchor + obi_shift - inv_shift

            # Vol-aware dynamic half-spread: widen quotes when realised vol
            # exceeds vol_cap (Avellaneda-Stoikov style). half_spread floors at
            # the configured min, scales with rv when rv > vol_cap.
            min_half = cfg["half_spread"]
            if sigma_ret > vol_cap and vol_cap > 0:
                half = max(min_half, K_VOL * sigma_ret)
            else:
                half = float(min_half)
            ...
            # Take-and-make tested with two reference points (OBI-shifted fair,
            # mu) and both bled −600k+ on 3 days. Mechanism: mu lags trend, so
            # `ask < mu - threshold` fires during downtrends — we BUY the
            # falling market. OBI-shifted fair drifts even more aggressively.
            # Take-and-make on a single product is a stat-arb tool (cross-product
            # reference), not a single-product passive-MM extension. Disabled.

            # ---- Maker sizing (asym. tilt + inventory taper) ----
