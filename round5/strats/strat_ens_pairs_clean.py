...
C_CORR_PRODUCTS: List[str] = SNACKPACK + [SLP_COT, SLP_POL]

# Pairs: (a, b, weight, unit, ent_z, ext_z)
# SNACKPACK: 5 cointegrated pairs (Engle-Granger p<0.05). RASP is anchor (in 4 pairs).
# CHOC<->VAN dropped (p=0.462, no stationary spread).
# PEBBLES pairs all dropped (p in [0.338, 0.625] - level-corr trend lag, not mean-rev).
# slp_cp unit=5 per 555509 (live winner).
C_PAIRS: List[Tuple[str, str, float, int, float, float]] = [
    (CHOC, RASP, -1.0, 2, 2.0, 0.5),    # NEW p=0.007
    (VAN,  RASP, -1.0, 2, 2.0, 0.5),    # NEW p=0.007
    (PIST, RASP, -1.0, 2, 2.0, 0.5),    # p=0.022
    (PIST, STRAW, +1.0, 3, 2.0, 0.5),   # p=0.031
    (RASP, STRAW, -1.0, 3, 2.0, 0.5),   # p=0.024
...
# fair = const + Σ wᵢ·midᵢ on dominant GALAXY feature.
# |z| > z_thr → take aggressively against fair; else post inside fair ± dyn
# with inventory skew. Position-limit 10 each. Pair products
# (PLANETARY_RINGS, SOLAR_WINDS) are READ ONLY here — features in oracle,
# not traded. BLACK_HOLES still trades via Block C (smart MM + OBI).
#
# Simplified to 1-feature regressions per cor_mid analysis: only the
# dominant feature (|cor|>0.3) carries signal; other coeffs were noise
# (|w|<0.06). Const unchanged; online μ_r absorbs feature-set shift
# within F_RESID_WARMUP=50 ticks (no offline re-fit, avoids BT-as-optimizer).
F_GALAXY_ORACLES: Dict[str, Dict[str, Any]] = {
    "GALAXY_SOUNDS_SOLAR_FLAMES": {
        ...
        "const": 14350.293324,
        "weights": {
            "GALAXY_SOUNDS_SOLAR_WINDS": -0.300809,  # cor_mid -0.335
        },
    },
    "GALAXY_SOUNDS_DARK_MATTER": {
        ...
        "const": 8960.332147,
        "weights": {
            "GALAXY_SOUNDS_PLANETARY_RINGS": 0.190165,  # cor_mid +0.434
        },
    },
}
