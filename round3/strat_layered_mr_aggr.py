"""Phase 6d: aggressive variant of strat_layered_mr.py.

Same architecture, but tuned for maximum backtest PnL via parameter sweep:

  HL_TICKS = 10000   (~ full historical day; effectively the opening level)
  SKEW_FACTOR = 3
  SKEW_CLIP = 5

3-day backtest total: +101,186  (--match-trades worse)
Per-day:              +37,894 / +38,886 / +24,406

Caveat: HL=10000 is the full backtest day. The "long-term mean" is essentially
the opening price; the strategy bets that intraday VE drift mean-reverts to
the open by end-of-day. Robust on the 3 historical days, but the assumption
may not hold across regimes — keep `strat_layered_mr.py` (HL=2000) as the
defensive default and use this only if backtest consistency is verified.
"""
